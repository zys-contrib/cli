...
Installer.prototype.readLocalPackageData = function (cb) {
  ...
  mkdirp(this.where, iferr(cb, function () {
    readPackageTree(self.where, iferr(cb, function (currentTree) {
      ...
      let metadata = {}
      try {
        metadata = require(path.join(self.where, 'node_modules', '.dependency-info.json'))
      } catch (err) {}
      currentTree.links = metadata.links || {
        // TODO - remove test data
        dep: {
          range: '*',
          realpath: '/Users/zkat/Documents/code/test/florp/dep'
        }
      }
